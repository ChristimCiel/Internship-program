...
SEO_Writer = Agent(
    ...
    goal="你是一位專業的 SEO 寫手，每篇文章的內容長度需控制在 3000 ~ 3500 token，以詼諧幽默、雙關語、以及當下網路流行語來撰寫，"
        ...
        "必須具備可靠的資料來源、納入與 {topic} 相關的延伸關鍵字和核心關鍵字，以提升在 Google Search 中的搜尋排名。"
        ...
        "3.	延伸撰寫：以 Researcher_Agent 和 AIMochi_Agent 所提供的內容為基礎，針對 {topic} 進行延伸撰寫，使文章具有深度並能覆蓋相關主題。", 
...
SEO_Writer_Bot = Task(
    description=(
        "作為一位資深的 SEO 寫手，你的目標是根據前述所提供的內容，撰寫出一篇圍繞 {topic} 的專業 SEO 文章。"
	...
	"標題 (10-15字)"
        ...
        "•	設計吸引讀者點擊的標題"
	"1. 起 (25%)"
        "•	使用吸引眼球的開場白或提出問題以引出 {topic}"
        "•	介紹一個與 {topic} 相關的有趣現象或最新趨勢"
        "•	運用故事化敘述（20%）抓住讀者的注意力"
        "•	簡述文章的主要內容並暗示將介紹的解決方案"
	"2. 承 (35%)"
        "•	透過 Researcher_Agent 和 AIMochi_Agent 的內容，提出具體案例或小故事以深入探討"
        "•	使用對比手法（13%）強調 {topic} 的關鍵價值或獨特之處"
        "•	提供與 {topic} 高度相關的連結和參考資料，增強讀者信任感"
        "•	適當使用擬人化表達（4%），增強情感共鳴"
        "•	使用互動性提問（3%），引導讀者參與思考"
	"3. 轉 (25%)"
	...
	"4. 合 (15%)"
        ...
        "•	得出對 {topic} 的結論或提出更大的洞見"
        "•	留下思考空間或發出行動呼籲，引導讀者進行下一步"

    "注意事項："

        "•	段落結構：不要將「起」「承」「轉」「合」這些詞明顯標出，而是依循段落設計的邏輯進行撰寫。"
        "•	引用連結：確保內文包含參考資料或外部連結，增強文章的權威性。"
        "•	語氣風格：語氣詼諧幽默，使用雙關語及網路流行語，以生動有趣的方式呈現專業知識。"
        "•	文案連貫性：確保各部分之間過渡自然，故事和例子引人入勝，能引起讀者共鳴。"
        "•	語言使用：所有內容需以繁體中文（台灣）撰寫，可適當加入少量英語詞句以增添趣味，但主要讀者為台灣地區。",
